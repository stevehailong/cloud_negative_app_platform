  ...
  <div class="secret-list">
      ...
      >
        ...
        <el-table-column label="类型" width="150">
          <template #default="{ row }">
            <el-tag>{{ row.secretType || 'Opaque' }}</el-tag>
          </template>
        </el-table-column>
    ...
    >
      ...
      >
        <el-form-item label="名称" prop="name">
          <el-input v-model="formData.name" placeholder="请输入Secret名称" />
        </el-form-item>
        ...
        <el-form-item label="Secret类型" prop="secretType">
          <el-select v-model="formData.secretType" placeholder="请选择类型" style="width: 100%">
            <el-option label="Opaque (普通密钥)" value="Opaque" />
            <el-option label="TLS (TLS证书)" value="kubernetes.io/tls" />
            <el-option label="Docker Config (镜像拉取)" value="kubernetes.io/dockerconfigjson" />
          </el-select>
        </el-form-item>
        
        <el-form-item label="密钥数据" prop="data">
          <el-input
            v-model="formData.data"
            type="textarea"
            :rows="8"
            placeholder='JSON格式密钥数据(值需base64编码), 如: {"username": "YWRtaW4=", "password": "MTIzNDU2"}'
          />
    ...
    >
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="Secret类型" :span="2">
          <el-tag>{{ currentItem.secretType || 'Opaque' }}</el-tag>
        ...
        <el-descriptions-item label="密钥数据" :span="2">
          <el-alert type="warning" :closable="false" style="margin-bottom: 10px">
            密钥数据已加密显示，实际值已base64编码
          </el-alert>
          <pre v-if="currentItem.data" style="margin: 0">{{ formatSecretData(currentItem.data) }}</pre>
          <span v-else>-</span>
        </el-descriptions-item>
...
const dialogTitle = ref('新建Secret')
...
const rules = {
  name: [{ required: true, message: '请输入Secret名称', trigger: 'blur' }],
  envId: [{ required: true, message: '请选择环境', trigger: 'change' }],
  namespace: [{ required: true, message: '请输入命名空间', trigger: 'blur' }],
  secretType: [{ required: true, message: '请选择Secret类型', trigger: 'change' }],
  data: [
    { required: true, message: '请输入密钥数据', trigger: 'blur' },
    {
      validator: (rule, value, callback) => {
        try {
          ...
          callback(new Error('密钥数据必须是有效的JSON格式'))
        }
      },
      trigger: 'blur'
    }
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await request.get('/secrets', { params })
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (formData.id) {
      await request.put(`/secrets/${formData.id}`, formData)
      ElMessage.success('更新成功')
    } else {
      await request.post('/secrets', formData)
...
const handleDelete = async (id) => {
  try {
    await ElMessageBox.confirm('确定要删除该Secret吗？', '提示', {
      type: 'warning'
    })
    await request.delete(`/secrets/${id}`)
