...
// SyncCostData 从 Prometheus 同步成本数据
func (h *CostHandler) SyncCostData(c *gin.Context) {
	...
	if err != nil {
		log.Printf("[CostHandler] SyncFromPrometheus error: %v", err)
		response.Error(c, response.CodeExternalAPIError, "同步成本数据失败: "+err.Error())
		return
	}

	log.Printf("[CostHandler] Synced %d cost records for cluster=%d date=%s", synced, req.ClusterID, req.CostDate)
